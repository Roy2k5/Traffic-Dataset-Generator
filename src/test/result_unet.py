...
result_unet.py
==============
Visualize UNet segmentation results on 5 sample test images.

For each image, saves a side-by-side figure:
    [Original RGB] | [Ground-Truth Mask] | [Predicted Mask]

Output saved to: src/test/results_unet/

Run from project root:
    python src/test/result_unet.py
"""
...
CKPT_PATH   = os.path.join(ROOT, 'src', 'checkpoint', 'unet', 'best.pth')
OUT_DIR     = os.path.join(ROOT, 'src', 'test', 'results_unet')
IMG_SIZE    = (256, 256)   # must match train_unet.py

# 5 images from the test split (index 1000–1199)
SAMPLE_INDICES = [1000, 1030, 1060, 1100, 1150]

CLASS_NAMES  = ['background', 'car', 'tree', 'lamppost']
# Colour map: background=black, car=red, tree=green, lamppost=yellow
...
def build_model(ckpt_path, device):
    model = UNet(n_channels=3, n_classes=NUM_SEG_CLASSES, bilinear=False)
    ckpt  = torch.load(ckpt_path, map_location=device, weights_only=False)
    model.load_state_dict(ckpt['model_state_dict'])
    epoch = ckpt.get('epoch', '?')
    miou  = ckpt.get('val_miou', float('nan'))
    print(f'Loaded UNet checkpoint: epoch={epoch}, val_mIoU={miou:.4f}')
    model.to(device).eval()
    return model


def mask_to_rgb(mask_np, colors):
    """Convert (H,W) class-index mask to (H,W,3) RGB."""
    h, w = mask_np.shape
...
def main():
    ...
    normalize = T.Normalize(mean=_GENERIC_MEAN, std=_GENERIC_STD)

    # Load GT semantic masks (precomputed)
    ...
    for idx in SAMPLE_INDICES:
        ...
        img_tensor  = TF.to_tensor(img_resized)          # (3, H, W)
        ...
        if has_gt:
            gt_path = os.path.join(sem_dir, f'{idx:04d}.npy')
            gt_mask = np.load(gt_path)
            # Resize GT to same display size
            gt_pil  = Image.fromarray(gt_mask, mode='L').resize(IMG_SIZE, Image.NEAREST)
            gt_np   = np.array(gt_pil)
            gt_rgb  = mask_to_rgb(gt_np, CLASS_COLORS)
            ncols   = 3
        else:
            ncols = 2
        ...
        fig.suptitle(f'UNet Segmentation — Image {idx:04d}', fontsize=16, y=1.02)
